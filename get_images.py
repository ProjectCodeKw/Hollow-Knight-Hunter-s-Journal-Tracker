...
hunter_journal_kills = {
    ...
    "Furious Vengefly": [0, 15, "Infected Crossroads & C.O.F"],
...
enemies = [
        ...
        {"code_name": "killedPrayerSlug", "game_name": "???"},
...
expected_files = [
    normalize_name(name) for name in hunter_journal_kills.keys()
]

# Get actual files in the folder (without extension)
actual_files = [
    os.path.splitext(f)[0] for f in os.listdir(SAVE_DIR) if os.path.isfile(os.path.join(SAVE_DIR, f))
]
...
l = ["Husk Sentry", "Zote", "Grey Prince Zote", "Oblobble", "White Defender", "Shielded Fool"]
for i in l:
